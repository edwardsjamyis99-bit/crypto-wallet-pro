...
function App() {
  ...
  const fetchRates = async () => {
    try {
      const response = await axios.get(`${API_URL}/rates/all`);
  ...
  const fetchWallets = async (uid) => {
    try {
      const response = await axios.get(`${API_URL}/wallets/${uid}`);
  ...
  const createWallet = async () => {
    ...
    try {
      const response = await axios.post(`${API_URL}/wallet/create`, {
        userId,
        chain: selectedChain,
        name: newWalletName || `${selectedChain} Wallet`
      });
  ...
  const sendTransaction = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`${API_URL}/transaction/send`, {
        fromWalletId: selectedWallet.id,
        toAddress: sendAddress,
        amount: parseFloat(sendAmount),
        currency: selectedWallet.chain,
        note: sendNote,
        speed: sendSpeed
      });
      
      alert('Transaction sent successfully! Receipt generated.');
      await fetchWallets(userId);
  ...
  const receiveTransaction = async (e) => {
    e.preventDefault();
    if (!selectedWallet) return;
    
    setLoading(true);
    try {
      const response = await axios.post(`${API_URL}/transaction/receive`, {
        walletId: selectedWallet.id,
        fromAddress: receiveFrom,
  ...
  const getRateForWallet = (wallet) => {
    ...
    const rateKey = mapping[wallet.chain];
    return rates[rateKey]?.usd || 0;
  };

  const getTotalBalance = () => {
    return wallets.reduce((total, wallet) => {
      const rate = getRateForWallet(wallet);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-50">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right">
                ...
                <div className="text-xl font-bold">${getTotalBalance().toFixed(2)}</div>
              </div>
              <button
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
            ...
            {wallets.map(wallet => (
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-sm opacity-90">{wallet.balance.toFixed(4)} {wallet.chain}</div>
                </div>
              </button>
            ))}
            <button
              onClick={() => setActiveTab('create-wallet')}
              className="flex items-center space-x-2 px-4 py-3 bg-white rounded-xl hover:shadow-md border-2 border-dashed"
            >
        ...
        {selectedWallet && (
          <>
            {/* Main Balance Card */}
            <div className="bg-gradient-to-r from-purple-600 to-blue-600 rounded-2xl p-6 text-white mb-6 shadow-xl">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-purple-100 mb-1">{selectedWallet.name}</p>
                  <p className="text-3xl font-bold mb-2">
                    {selectedWallet.balance.toFixed(4)} {selectedWallet.chain}
                  </p>
                  <p className="text-purple-100">
                    ≈ ${(selectedWallet.balance * getRateForWallet(selectedWallet)).toFixed(2)} USD
                  </p>
                </div>
                <div className="text-right">
            ...
            <div className="grid grid-cols-4 gap-4 mb-6">
              <button
                ...
                className="bg-white p-4 rounded-xl text-center hover:shadow-lg transition-all"
              >
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
              {activeTab === 'send' && (
                <form onSubmit={sendTransaction}>
                  <h2 className="text-xl font-bold mb-4">Send {selectedWallet.chain}</h2>
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-2">Amount ({selectedWallet.chain})</label>
                      <input
                        type="number"
                        step="any"
                        value={sendAmount}
              ...
              {activeTab === 'receive' && (
                <div>
                  <h2 className="text-xl font-bold mb-4">Receive {selectedWallet.chain}</h2>
                  ...
                  <form onSubmit={receiveTransaction}>
                    <h3 className="font-semibold mb-3">Manual Entry (Record Receipt)</h3>
                    <div className="space-y-4">
                      <div>
                          ...
                          <div className="flex justify-between items-start">
                            <div>
                              <div className="flex items-center space-x-2 mb-2">
                                <span className={`text-lg ${tx.type === 'send' ? 'text-red-500' : 'text-green-500'}`}>
                                  {tx.type === 'send' ? '📤' : '📥'}
                                </span>
                            ...
                            <div className="text-right">
                              ...
                              {tx.fiatValue && (
                                <p className="text-sm text-gray-500">≈ ${tx.fiatValue.toFixed(2)}</p>
                              )}
                              {tx.txHash && (
                                <button
              ...
              {activeTab === 'create-wallet' && (
                <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium mb-2">Select Blockchain</label>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">Receive {selectedWallet.chain}</h3>
              <button onClick={() => setShowQR(false)} className="text-gray-500">✕</button>
            </div>
            <div className="text-center">
              <div className="bg-gray-100 p-4 rounded-lg mb-4">
                <p className="font-mono text-sm break-all">{selectedWallet.address}</p>
              </div>
              <p className="text-sm text-gray-600">Scan this QR code to receive payments</p>
              <button
                onClick={() => navigator.clipboard.writeText(selectedWallet.address)}
